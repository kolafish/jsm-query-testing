...
Run time: 2026-05-11T11:43:15.852480+00:00
...
## Weighted Cop Stress Summary

This run keeps all `22` runnable query classes, but assigns extra workers to the slow / TiKV cop-heavy query classes: `4, 5, 8, 10, 14, 15, 16, 17, 19, 21, 23, 24, 25`.

| Item | Value |
|---|---:|
| Concurrency | 220 workers |
| Duration window | 600s target, 626.6s elapsed including tail drain |
| Successful ops | 226,731 |
| Errors | 0 |
| QPS | 361.86 |
| Avg / P95 / Max latency | 592.7 ms / 587.7 ms / 104,873.0 ms |
| TiKV CPU avg / max | 17.51 / 24.62 cores, max 51.3% of 48 cores |
| TiDB CPU avg / max | 5.36 / 7.75 cores |
| TiFlash CPU avg / max | 76.64 / 84.76 cores |
| TiKV cop wait avg | 546.8 ms |
| TiKV cop handle avg | 7.8 ms |
| TiKV cop request duration avg | 1,101.4 ms |
| TiKV waiting for semaphore avg / max | 2,631.8 / 4,372 |
| TiKV select seek / next | 801,627/s / 919,923/s |
| TiKV select seek-to-next ratio | 0.87 |

Observation: this weighted run reproduces the customer's main TiKV symptom. TiKV CPU is not saturated, while cop wait is much larger than cop handle time and `tikv_coprocessor_waiting_for_semaphore` is high. The selected `seek` and `next` rates are also close enough to show seek-heavy scan behavior. Because this run intentionally keeps all query classes, TiFlash is also busy; the TiKV cop bottleneck should be interpreted from the TiKV-specific wait/handle/semaphore metrics rather than total cluster CPU alone.
...
|---:|---:|---:|---:|---:|---:|---:|---:|
| 220 | 22 | 600 | 226731 | 361.86 | 592.7 | 587.7 | 104873.0 |
...
- Window: `2026-05-11T11:32:48.959917+00:00` to `2026-05-11T11:43:15.529977+00:00`
- Grafana time range: [http://a2e41aa49d08647d1b55ecd7b146bbf6-2611d84fa96ba26a.elb.us-east-2.amazonaws.com:3000?from=1778499168959&to=1778499795529](http://a2e41aa49d08647d1b55ecd7b146bbf6-2611d84fa96ba26a.elb.us-east-2.amazonaws.com:3000?from=1778499168959&to=1778499795529)
...
|---|---:|---:|---:|---:|---:|---:|---|
| tidb | 3 | 5.36 | 7.75 | 16.1% | 33.09 | 42.87 |  |
| tikv | 3 | 17.51 | 24.62 | 51.3% | 59.66 | 60.83 |  |
| tiflash | 6 | 76.64 | 84.76 | 88.3% | 99.94 | 116.13 | TiFlash proxy process CPU metric |

Grafana panel screenshots:

<table>
<tr>
<td width="50%"><strong>TiDB CPU</strong><br><img src="images/jsm_assets4_concurrency220_tidb_cpu.png" alt="TiDB CPU - concurrency 220" /></td>
<td width="50%"><strong>TiDB Memory</strong><br><img src="images/jsm_assets4_concurrency220_tidb_memory.png" alt="TiDB Memory - concurrency 220" /></td>
</tr>
<tr>
<td width="50%"><strong>TiKV CPU</strong><br><img src="images/jsm_assets4_concurrency220_tikv_cpu.png" alt="TiKV CPU - concurrency 220" /></td>
<td width="50%"><strong>TiKV Memory</strong><br><img src="images/jsm_assets4_concurrency220_tikv_memory.png" alt="TiKV Memory - concurrency 220" /></td>
</tr>
<tr>
<td width="50%"><strong>TiFlash CPU</strong><br><img src="images/jsm_assets4_concurrency220_tiflash_cpu.png" alt="TiFlash CPU - concurrency 220" /></td>
