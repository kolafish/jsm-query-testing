...
Run time: 2026-05-11T12:48:46.071513+00:00
...
## Weighted Cop Stress Rerun Summary

This rerun uses the 16 vCPU / 64 GiB node configuration for TiDB, TiKV, and TiFlash: `3 TiDB / 3 TiKV / 6 TiFlash`. The benchmark client ran inside Kubernetes as `jsm-bench-runner`.

### 220 vs 330 Key Metrics

| Concurrency | QPS | Avg latency | TiKV cop wait avg | TiKV cop wait max |
|---:|---:|---:|---:|---:|
| 220 | 345.84 | 630.1 ms | 1,118.8 ms | 1,413.0 ms |
| 330 | 321.70 | 1,013.4 ms | 2,021.2 ms | 2,998.9 ms |

Pre-run validation:
- `jsm_assets4.obj_new = 14,000,000`, `obj_relationship_new = 139,988,146`, `obj_type = 50`, `obj_type_attr = 1,750`.
- The first smoke run found `Query 8` failing with `GetShardLocalCacheInfo failed: 13` because TiCI readers were not registered after meta restart.
- Restarted `tici-worker-0` and all 6 TiFlash pods; TiCI meta then registered 6 reader nodes.
- Final smoke run passed all 22 runnable query classes with zero errors.

| Concurrency | Successful ops | Errors | QPS | Avg ms | P95 ms | Max ms | TiKV cop wait avg/max | TiKV cop suspend avg/max | TiKV waiting semaphore avg/max | Select seek/s | Select next/s | Seek/next |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 220 | 211,211 | 0 | 345.84 | 630.1 | 755.3 | 78,739.5 | 1,118.8 / 1,413.0 ms | 1,118.0 / 1,412.2 ms | 3,044.4 / 4,542.0 | 959,796.0 | 1,202,101.8 | 0.80 |
| 330 | 197,674 | 0 | 321.70 | 1,013.4 | 1,252.8 | 117,135.7 | 2,021.2 / 2,998.9 ms | 2,019.7 / 2,998.7 ms | 5,637.9 / 8,369.0 | 1,166,524.7 | 1,180,843.9 | 0.99 |

Observation: increasing from 220 to 330 workers increased TiKV cop wait and semaphore pressure, but QPS dropped from `345.84` to `321.70`. This reproduces the high cop wait / wait-suspend behavior without TiKV CPU saturation: 330 workers reached about `68.8%` of TiKV CPU capacity while average cop wait was about `2.0s`.
...
|---:|---:|---:|---:|---:|---:|---:|---:|
| 220 | 22 | 600 | 211211 | 345.84 | 630.1 | 755.3 | 78739.5 |
| 330 | 22 | 600 | 197674 | 321.7 | 1013.4 | 1252.8 | 117135.7 |
...
- Window: `2026-05-11T12:27:50.308309+00:00` to `2026-05-11T12:38:01.019091+00:00`
- Grafana time range: [http://a2e41aa49d08647d1b55ecd7b146bbf6-2611d84fa96ba26a.elb.us-east-2.amazonaws.com:3000?from=1778502470308&to=1778503081019](http://a2e41aa49d08647d1b55ecd7b146bbf6-2611d84fa96ba26a.elb.us-east-2.amazonaws.com:3000?from=1778502470308&to=1778503081019)
...
|---|---:|---:|---:|---:|---:|---:|---|
| tidb | 3 | 5.28 | 7.09 | 14.8% | 45.23 | 52.92 |  |
| tikv | 3 | 22.55 | 29.64 | 61.8% | 90.56 | 108.33 |  |
| tiflash | 6 | 77.41 | 86.22 | 89.8% | 83.03 | 99.89 | TiFlash proxy process CPU metric |

| Query | Successful ops | Source avg | Source max | Source rows avg | Run avg ms | vs source avg | P95 ms | Max ms | Run rows avg |
|---:|---:|---:|---:|---:|---:|---|---:|---:|---:|
| 2 | 2479 | 320.8ms | 14332.3ms | 234 | 1213.7 | 3.78x source | 2092.1 | 29154.5 | 600 |
| 4 | 598 | 11882.9ms | 34102.7ms | 1000 | 17317.2 | 1.46x source | 36907.3 | 43941.9 | 1000 |
| 5 | 438 | 29101.7ms | 60034.6ms | 65 | 23562.5 | 1.24x faster | 38720.5 | 45799.9 | 1000 |
| 6 | 3861 | 44.8ms | 927.9ms | 168 | 777.8 | 17.36x source | 1494.0 | 2971.7 | 50 |
| 7 | 40154 | 8.7ms | 487.0ms | 35 | 74.7 | 8.59x source | 159.1 | 785.2 | 35 |
| 8 | 359 | 16983.6ms | 32291.9ms | 6 | 28587.1 | 1.68x source | 56122.8 | 69311.3 | 103.2 |
| 9 | 37204 | 28.1ms | 10778.1ms | 8 | 64.5 | 2.30x source | 136.5 | 1687.2 | 24 |
| 10 | 375 | 41204.1ms | 51112.0ms | 136 | 20947.0 | 1.97x faster | 36307.9 | 78739.5 | 955.1 |
| 11 | 3399 | 17.0ms | 228.7ms | 1 | 706.5 | 41.56x source | 1340.1 | 2302.7 | 1 |
| 13 | 20984 | 29.8ms | 6157.8ms | 175 | 114.4 | 3.84x source | 236.2 | 3266.0 | 175 |
| 14 | 438 | 28809.4ms | 45421.2ms | 1000 | 17993.2 | 1.60x faster | 36564.7 | 47617.4 | 1000 |
| 15 | 431 | 24650.8ms | 42060.9ms | 1000 | 18281.8 | 1.35x faster | 37610.7 | 49448.9 | 1000 |
| 16 | 353 | 47800.9ms | 60019.8ms | 500 | 22239.0 | 2.15x faster | 34454.8 | 44633.1 | 1000 |
| 17 | 422 | 29564.9ms | 37139.7ms | 0 | 18701.9 | 1.58x faster | 32717.6 | 69324.6 | 1000 |
| 18 | 56455 | 3.7ms | 325.7ms | 4 | 42.5 | 11.49x source | 99.2 | 374.7 | 5 |
| 19 | 371 | 39798.6ms | 60018.5ms | 500 | 21241.6 | 1.87x faster | 40976.0 | 52843.1 | 1000 |
| 20 | 38201 | 17.8ms | 5900.3ms | 1 | 62.8 | 3.53x source | 136.8 | 579.9 | 1 |
| 21 | 180 | 23577.6ms | 30859.8ms | 1000 | 43907.3 | 1.86x source | 58586.4 | 67393.8 | 1000 |
| 22 | 3272 | 25.9ms | 233.2ms | 20 | 733.6 | 28.32x source | 1413.6 | 2275.3 | 20 |
| 23 | 419 | 33550.1ms | 35378.6ms | 0 | 18792.3 | 1.79x faster | 35946.4 | 78307.6 | 959.8 |
| 24 | 457 | 60085.2ms | 60085.2ms | 0 | 17154.9 | 3.50x faster | 22225.9 | 28648.0 | 1000 |
| 25 | 361 | 59691.1ms | 59691.1ms | 1000 | 21849.6 | 2.73x faster | 30438.8 | 39338.6 | 1000 |

#### Concurrency 330

- Window: `2026-05-11T12:38:16.323068+00:00` to `2026-05-11T12:48:30.790174+00:00`
- Grafana time range: [http://a2e41aa49d08647d1b55ecd7b146bbf6-2611d84fa96ba26a.elb.us-east-2.amazonaws.com:3000?from=1778503096323&to=1778503710790](http://a2e41aa49d08647d1b55ecd7b146bbf6-2611d84fa96ba26a.elb.us-east-2.amazonaws.com:3000?from=1778503096323&to=1778503710790)
...
| Component | Replicas | CPU avg cores | CPU max cores | CPU max % capacity | Mem avg GiB | Mem max GiB | Note |
|---|---:|---:|---:|---:|---:|---:|---|
