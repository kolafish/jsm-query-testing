...
| 4. Relationship Traversal / Depth 1 / Query 4 | `text_value_1` | idx_obj_new_text_value_1_ngram (NGRAM) | 0 | 1210.0 | 0 | 935.9 | newly restored relationship query; the better rewrite pushes `UNION DISTINCT` into the FTS candidate-id subquery so the relationship traversal runs only once |
...
- Among the newly restored queries, `4. Relationship Traversal / Depth 1 / Query 4` is the only additional `LIKE` case whose target column already has a matching FULLTEXT index. The best rewrite is to `UNION DISTINCT` the small FTS candidate-id set first and then join/filter once; that version still returns 0 rows, but its runtime is close to the original `LIKE` form instead of much worse.
