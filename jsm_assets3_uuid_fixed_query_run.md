...
| 4. Relationship Traversal / Depth 1 / Query 4 | `text_value_1` | idx_obj_new_text_value_1_ngram (NGRAM) | 0 | 1210.0 | 0 | 1520.0 | newly restored relationship query; the `%...%` branch can be rewritten to `MATCH`, but it is slower on this dataset |
...
- Among the newly restored queries, `4. Relationship Traversal / Depth 1 / Query 4` is the only additional `LIKE` case whose target column already has a matching FULLTEXT index. The rewrite runs correctly, but it is slower than the original `LIKE` form and still returns 0 rows.
